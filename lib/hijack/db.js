...
hijackDBOps = function hijackDBOps() {
  ...
  const mongoCollectionProto = Mongo.Collection.prototype; // for aggregate
  if (mongoCollectionProto.aggregate) {
    const oldAggregate = mongoCollectionProto.aggregate;
    mongoCollectionProto.aggregate = function(pipeline) {
      var payload = {
        coll: this._name,
        func: "aggregate",
      ...
      //see: issue #6
      try{
        var ret = oldAggregate.apply(this, arguments);
        if (kadiraInfo && eventId && _.isArray(ret)) {
          Kadira.tracer.eventEnd(kadiraInfo.trace, eventId, { docsFetched: ret.length });
        }
      } catch(ex) {
        if(kadiraInfo && eventId) {
          ...
          if(type == 'fetch') {
            ...
            if(kadiraInfo) {
              if(kadiraInfo.trace.type === 'method') {
                ...
                Kadira.models.pubsub.trackPolledDocuments(kadiraInfo.trace, ret.length);
