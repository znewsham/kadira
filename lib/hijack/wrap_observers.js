/* if (Package["cultofcoders:redis-oplog"]) {
  const RedisSubscriber = require("meteor/cultofcoders:redis-oplog/lib/redis/RedisSubscriber.js").default;
  const oldProcess = RedisSubscriber.prototype.process;
  RedisSubscriber.prototype.process = function(...args) {
    oldProcess.call(this, ...args);
    if (this.observableCollection.cursorDescription && this.observableCollection.cursorDescription._subNames) {
      this.observableCollection.cursorDescription._subNames.forEach((o) => {
        Kadira.models.pubsub.trackDocumentChanges({ name: o }, { op: args[0] });
      });
    }
  };
} */
...
wrapForCountingObservers = function() {
  ...
  mongoConnectionProto._observeChanges = function(cursorDescription, ordered, callbacks) {
    ...
    if (!Kadira.env.currentCursorDescriptions) {
      Kadira.env.currentCursorDescriptions = [];
    }
    Kadira.env.currentCursorDescriptions.push(cursorDescription);
