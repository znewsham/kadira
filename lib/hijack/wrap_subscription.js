...
wrapSubscription = function(subscriptionProto) {
  ...
  subscriptionProto.ready = function() {
    ...
    if (Kadira.env.currentCursorDescriptions) {
      Kadira.env.currentCursorDescriptions.forEach((cd) => {
        if (!cd._subNames) {
          cd._subNames = new Set();
        }
        if (this._name) {
          cd._subNames.add(this._name);
        }
      });
  ...
  ['added', 'changed', 'removed'].forEach(function(funcName) {
    ...
    subscriptionProto[funcName] = function(collectionName, id, fields) {
      ...
      if (funcName === "added") {
        Kadira.models.pubsub.trackLiveUpdates({ name: this._name }, this._apmReadyTracked ? "_addPublished" : "_initialAdds", 1);
      }
      else if (funcName === "removed") {
        Kadira.models.pubsub.trackLiveUpdates({ name: this._name }, "_removePublished", 1);
      }
      else if (funcName === "changed") {
        Kadira.models.pubsub.trackLiveUpdates({ name: this._name }, "_changePublished", 1);
      }
